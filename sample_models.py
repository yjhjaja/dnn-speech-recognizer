...
from keras.layers import (BatchNormalization, Conv1D, Dense, Dropout, Input, 
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        temp = SimpleRNN(units, activation='relu', return_sequences=True, implementation=2, name='rnn_{}'.format(i))(temp)
        temp = BatchNormalization(name='bn_simp_rnn_{}'.format(i))(temp)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(temp)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride, conv_border_mode, units, output_dim=29):
    ...
    conv_1d = Conv1D(filters, kernel_size, 
                     ...
                     activation='relu',
                     name='conv1d')(input_data)
    drop = Dropout(.2)(conv_1d)
    bn_cnn = BatchNormalization(name='bn_conv_1d')(drop)
    
    temp = bn_cnn
    for i in range(2):
#         temp = SimpleRNN(units, activation='relu', return_sequences=True, implementation=2, name='rnn_{}'.format(i))(temp)
        
#         temp = Bidirectional(merge_mode='concat', name='bd_{}'.format(i))(temp)
        
        temp = Bidirectional(SimpleRNN(units, activation='relu', return_sequences=True, implementation=2), merge_mode='concat')(temp)
